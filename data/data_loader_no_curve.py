...
class Dataset_Font(Dataset):
    def __init__(self, root_path, flag='train', size=None, 
        ...
        # if flag != 'train_font':
        #     flag = 'val_font'
        # file = open('/home/defuc/sensei-fs-symlink/users/defuc/workspace/SVG_Informer_name__cls_distance/less_name_'+flag +'.pkl','rb')
        # less_name = pkl.load(file)
        # file.close()

        for i in range(len(data_pkl)):
            if len(data_pkl[i])>self.max_length:
                continue
            # if not data_name[i] in less_name:
            #     continue
            ...
            self.curve_matrix.append(['1'])
            d_c = 0 #np.array(data_curve[data_name[i]])
